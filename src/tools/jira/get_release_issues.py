...
Examples:
    ...
    parser.add_argument("--project", default=None, help="Project key (uses DB/ENV setting if not specified)")
    parser.add_argument("--current-sprint", action="store_true", help="Get from current sprint")
    parser.add_argument("--max-results", type=int, default=100)

    args = parser.parse_args()

    # Resolve project key: CLI arg > DB setting > ENV var
    if not args.project:
        from src.utils.system_config import get_setting as _get_setting
        args.project = _get_setting("jira_project_key", env_fallback="ATLASSIAN_PROJECT_KEY") or ""
