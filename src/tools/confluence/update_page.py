...
from ..base import get_confluence_auth_headers, CONFLUENCE_BASE_URL, JIRA_WEB_URL, format_error
...
def update_confluence_page(
    ...
    try:
        ...
        if result["url"] and not result["url"].startswith("http"):
            result["url"] = f"{JIRA_WEB_URL}/wiki{result['url']}"
...
def add_feature_to_release_notes(
    ...
    feature_html = f"""
<h3>{feature_title}</h3>
<p><strong>Jira:</strong> <a href="{JIRA_WEB_URL}/browse/{jira_key}">{jira_key}</a></p>
<p>{description}</p>
"""
...
<td><a href="{JIRA_WEB_URL}/browse/{jira_key}">{jira_key}</a></td>
</tr>"""
